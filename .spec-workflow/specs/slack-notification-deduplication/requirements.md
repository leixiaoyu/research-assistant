...
2. IF a paper exists in the global registry (matched by DOI, paper_id, or title similarity ≥95%) AND has status `PROCESSED` or `MAPPED` THEN the system SHALL NOT count it in the `papers_discovered` metric for notifications
3. IF a paper exists in the global registry with status `FAILED` or `SKIPPED` THEN the system SHALL:
   - Count it as a "new" paper (to give it another chance)
   - Include it in a separate "Retry Papers" section if `show_retry_papers` is enabled
   - This ensures failed papers are not silently lost and get user visibility
4. WHEN the notification is sent THEN the system SHALL include a breakdown showing:
   - `new_papers`: Papers discovered for the first time
   - `retry_papers`: Previously failed papers being retried (optional)
   - `duplicate_papers`: Papers already successfully processed (optional, for transparency)
...
1. WHEN configuring notifications THEN the system SHALL support the following options:
   ...
   - `show_retry_papers`: Boolean to show papers being retried after previous failure (default: true)
...
- **Modular Design**: Create notification deduplication logic within `src/services/notification/` package (not in RegistryService) to maintain clean separation of concerns
- **Dependency Management**: Notification module should consume RegistryService as a dependency for paper identity lookup, not duplicate the logic
- **Clear Interfaces**: Define clear contract for "is paper new for notification purposes" with explicit handling of paper status (PROCESSED, FAILED, SKIPPED, MAPPED)
