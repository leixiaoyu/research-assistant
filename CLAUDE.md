...
**Phase 3 Modules (✅ Complete - Intelligence Layer):**
- **`src/services/cache_service.py`**: Multi-level caching (API, PDF, extraction results)
- **`src/services/dedup_service.py`**: Two-stage deduplication (DOI + fuzzy title matching)
- **`src/services/filter_service.py`**: Quality filtering and weighted ranking
- **`src/services/checkpoint_service.py`**: Atomic checkpoint saves for crash-safe resumption
...
* **SQLite write retry (canonical pattern, issue #133):** Wrap every `BEGIN IMMEDIATE` write site in `src.storage.intelligence_graph.connection.retry_on_lock_contention(operation, *, max_attempts=DEFAULT_MAX_ATTEMPTS, backoff_seconds=DEFAULT_BACKOFF_SECONDS, operation_name="...")`. The helper retries on `SQLITE_BUSY`/`SQLITE_LOCKED` (errorcode 5/6 with substring fallback), propagates non-contention `OperationalError` immediately, and emits standardized `sqlite_lock_contention_retry` (warning) / `sqlite_lock_contention_exhausted` (error) events. Pass a distinctive `operation_name` so a single audit query (`event:sqlite_lock_contention_*`) covers every write site. **Async callers MUST `await asyncio.to_thread(...)` around the wrapped call** — the helper sleeps via `time.sleep` and would otherwise block the event loop. Canonical caller: `MonitoringRunRepository.record_run`. Reviewer-side guidance: see `GEMINI.md` §5 "State Integrity & Durability". **New** BEGIN IMMEDIATE write sites must use `retry_on_lock_contention`. Pre-existing call sites are tracked in #134 (citation) and via TODOs in source for future migration. **Do not** use this helper for read-only paths or for writes whose idempotency depends on caller-side state being advanced between attempts.
