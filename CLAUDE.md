...
* **SQLite write retry (canonical pattern, issue #133):** Wrap every `BEGIN IMMEDIATE` write site in `src.storage.intelligence_graph.connection.retry_on_lock_contention(operation, *, max_attempts=3, backoff_seconds=0.05, operation_name="...")`. The helper retries on `SQLITE_BUSY`/`SQLITE_LOCKED` (errorcode 5/6 with substring fallback), propagates non-contention `OperationalError` immediately, and emits standardized `sqlite_lock_contention_retry` (warning) / `sqlite_lock_contention_exhausted` (error) events. Pass a distinctive `operation_name` so a single audit query (`event:sqlite_lock_contention_*`) covers every write site. **Async callers MUST `await asyncio.to_thread(...)` around the wrapped call** — the helper sleeps via `time.sleep` and would otherwise block the event loop. Canonical caller: `MonitoringRunRepository.record_run`. Reviewer-side guidance: see `GEMINI.md` §5 "State Integrity & Durability".
