...
>
> **Operator runbook — registry-warming period (REQ-9.5.2.1 / B-G1 cold start):** The first 7 days after PR α/β reach production are an expected "warming" window during which the recent quality-cohort cohort is sparse or empty (the registry is just beginning to accumulate post-PR-#157 high-quality extractions for any topic). During this window, `select_citation_seeds` returns an empty list and `_select_citation_seeds` falls back to the legacy `all_papers[:10]` seeding behavior — visible in logs as the `citation_seeds_using_fallback` event. Because legacy seeds overlap heavily with provider results, the breadth-metric rate will look low and `within_slo=False` is the expected per-run signal. **Do not page on `within_slo=False` until the registry has ≥7 days of high-quality entries for the topics in `research_config.yaml`.** Confirm warming completion by grepping daily-run logs for `citation_seeds_selected` (real cohort) replacing `citation_seeds_using_fallback` (cold fallback) per topic.
