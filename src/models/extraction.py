...
class ExtractionTarget(BaseModel):
    ...
    output_format: Literal["text", "code", "json", "list"] = Field(
        default="text",
        description="Expected format of extracted content"
    )
    required: bool = Field(
        default=False,
        description="Whether extraction must succeed (fail if not found)"
    )
...
class PaperExtraction(BaseModel):
    ...
    extraction_results: List[ExtractionResult] = Field(
        ...
        description="Results for each extraction target"
    )
...
class ExtractedPaper(BaseModel):
    ...
    pdf_path: Optional[str] = Field(
        default=None,
        description="Path to downloaded PDF file"
    )
    markdown_path: Optional[str] = Field(
        default=None,
        description="Path to converted markdown file"
    )
