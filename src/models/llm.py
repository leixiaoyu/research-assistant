...
class LLMConfig(BaseModel):
    ...
    model: str = Field(
        default="claude-3-5-sonnet-20250122",
    ...
    def validate_api_key(cls, v: str) -> str:
        ...
        if v in ['YOUR_API_KEY', 'PLACEHOLDER', '', 'None']:
            raise ValueError("API key must be a valid credential from environment variable")
        return v
...
class CostLimits(BaseModel):
    ...
    max_tokens_per_paper: int = Field(
        ...
        gt=0,
        le=200000,
        description="Maximum tokens to use per paper"
    )
    ...
    max_total_spend_usd: float = Field(
        ...
        gt=0.0,
        le=10000.0,
        description="Maximum total spending in USD"
    )
