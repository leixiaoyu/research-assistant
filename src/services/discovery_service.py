...
class DiscoveryService:
    """Wrapper service for research paper discovery with multi-provider support"""
    ...
    def __init__(self, api_key: str = ""):
        ...
        if api_key:
            self.providers[ProviderType.SEMANTIC_SCHOLAR] = SemanticScholarProvider(api_key=api_key)
        else:
    ...
    async def search(self, topic: ResearchTopic) -> List[PaperMetadata]:
        ...
        if provider_type not in self.providers:
            if provider_type == ProviderType.SEMANTIC_SCHOLAR:
                logger.error("provider_unavailable", provider=provider_type, reason="missing_api_key")
                raise APIError(f"Provider {provider_type} is configured but not available (missing API key). Check .env file.")
            else:
