...
def service(pdf_settings, mock_extractors):
        with patch("src.services.pdf_extractors.fallback_service.PyMuPDFExtractor", return_value=mock_extractors["pymupdf"]), \
             patch("src.services.pdf_extractors.fallback_service.PDFPlumberExtractor", return_value=mock_extractors["pdfplumber"]), \
             patch("src.services.pdf_extractors.fallback_service.PandocExtractor", return_value=Mock(validate_setup=Mock(return_value=False))):
...
async def test_fallback_to_second_backend(service, mock_extractors):
    ...
    success_result = PDFExtractionResult(
        success=True,
        markdown="Valid content",
        metadata={"backend": PDFBackend.PDFPLUMBER}
    )
