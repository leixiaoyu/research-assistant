...
@pytest.fixture
def mock_components():
    with patch("src.cli.ConfigManager") as MockConfig, \
         ...
         patch("src.cli.open", new_callable=MagicMock) as mock_open: # Mock file writing
        ...
        discovery_instance.search.return_value = [PaperMetadata(paper_id="1", title="P1", url="http://u", year=2023)]
...
def test_run_discovery_error(mock_components):
    ...
    # We can't easily check log output with CliRunner unless we capture stderr/stdout and logging is configured to print there.
