...
async def test_api_status_errors(provider, topic):
    ...
    with patch("src.services.providers.arxiv.feedparser.parse", return_value=mock_feed_403):
        try:
            await provider.search(topic)
    ...
    with patch("src.services.providers.arxiv.feedparser.parse", return_value=mock_feed_500):
        try:
            await provider.search(topic)
...
async def test_bozo_warning(provider, topic):
    ...
    with patch("src.services.providers.arxiv.feedparser.parse", return_value=mock_feed):
        with patch("src.services.providers.arxiv.logger") as mock_logger:
            await provider.search(topic)
            mock_logger.warning.assert_called_with("arxiv_feed_parse_warning", error="XML Error")
...
def test_build_query_timeframes(provider, topic):
    ...
    # Date Range
    topic.timeframe = TimeframeDateRange(start_date=date(2023,1,1), end_date=date(2023,1,2))
...
async def test_rate_limiting_enforces_3_second_delay():
    """
    SR-1.5-1: Runtime verification that ArXiv rate limiting enforces 3-second minimum delay.

    This test actually measures timing between consecutive requests to verify the
    rate limiter enforces the required 3-second delay between ArXiv API calls.
    """
    ...
    with patch("src.services.providers.arxiv.feedparser.parse", return_value=mock_feed):
        ...
        # Allow 0.1s tolerance for timing variations
        assert second_request_time >= 2.9, \
            f"Rate limiter failed: second request took {second_request_time}s, expected >=3s"

        # Verify it's not excessively delayed (sanity check)
        assert second_request_time < 4.0, \
            f"Rate limiter delayed too much: {second_request_time}s, expected ~3s"